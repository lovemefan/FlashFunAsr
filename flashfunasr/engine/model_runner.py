...
from flashfunasr.config import Config
from flashfunasr.engine.sequence import Sequence
from flashfunasr.modules.qwen3 import Qwen3ForCausalLM
from flashfunasr.modules.sampler import RasSampler, Sampler
from flashfunasr.utils.context import (get_context, reset_context,
                                          set_context)
from flashfunasr.utils.loader import load_model
...
class ModelRunner:
    ...
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        if self.world_size > 1:
            if rank == 0:
                self.shm = SharedMemory(name="flashfunasr", create=True, size=2**20)
                dist.barrier()
            else:
                dist.barrier()
                self.shm = SharedMemory(name="flashfunasr")
                self.loop()
    ...
    def warmup_model(self):
        ...
        seqs = [Sequence([0]*max_model_len, [torch.zeros(self.config.hf_config.hidden_size) ] * max_model_len) for _ in range(num_seqs)]
        self.run(seqs, True)
        torch.cuda.empty_cache()
    ...
    def prepare_prefill(self, seqs: list[Sequence]):
        ...
        for seq in seqs:
            ...
            # 2. 处理 Token IDs (如果有)
            if seqlen > start_pos:
                input_ids.extend(seq[start_pos:])

            # 3. 处理已有的 Embedding (如果有)
            # 假设 seq.input_embedding 也是按位置对应的
            if hasattr(seq, 'input_embedding') and seq.input_embedding is not None:
                # 只取尚未 cache 的那部分 embedding
                inputs_embeds_list.extend(seq.input_embedding[start_pos:])
    ...
    def prepare_decode(self, seqs: list[Sequence]):
        ...
        for seq in seqs:
            ...
            # inputs_embeds.append(seq.input_embedding[-1])
            inputs_embeds.append(self.model.model.embed_tokens(torch.tensor(seq.last_token,dtype=torch.int64).cuda()))
            positions.append(len(seq))
            context_lens.append(len(seq))
            slot_mapping.append(seq.block_table[-1] * self.block_size + seq.last_block_num_tokens - 1)

        # inputs_embeds = torch.concat((seq.input_embedding, torch.stack(inputs_embeds, dim=0)), dim=0)
    ...
    def run_model(self, inputs_embeds: torch.Tensor, positions: torch.Tensor, is_prefill: bool):
        if is_prefill or self.enforce_eager or inputs_embeds.size(0) > 512:
            return self.model.compute_logits(self.model(inputs_embeds, positions))
        else:
            ...
            for k, v in graph_vars.items():
                if k != "outputs":
                    v.zero_()
            graph_vars["inputs_embeds"][:bs] = inputs_embeds
    ...
    def run(self, seqs: list[Sequence], is_prefill: bool) -> list[int]:
        inputs_embeds, positions = self.prepare_prefill(seqs) if is_prefill else self.prepare_decode(seqs)
        if self.rank == 0 or self.world_size == 1:
            sample_params = self.prepare_sample(seqs)
            logits = self.run_model(inputs_embeds, positions, is_prefill)
    ...
    def capture_cudagraph(self):
        ...
        inputs_embeds = torch.zeros((max_bs, self.config.hf_config.hidden_size), dtype=torch.bfloat16)
        ...
        for bs in reversed(self.graph_bs):
            ...
            outputs[:bs] = self.model(inputs_embeds[:bs], positions[:bs])    # warmup
            with torch.cuda.graph(graph, self.graph_pool):
                outputs[:bs] = self.model(inputs_embeds[:bs], positions[:bs])    # capture
